...
- 新增首页入口：点击“开始测评”进入答题页，点击“查看英雄池”进入英雄池页；子页面使用简洁顶部导航，不再重复首页大标题
- 答完后会进入独立结果页，展示本命英雄、匹配度、游戏特征、相似点、性格定义、优缺点、生活建议、人格雷达和备选英雄
- 英雄池已经放到独立界面，支持搜索、职业筛选；点击某个英雄标签后会显示该英雄的 quote、desc 和算法人格标签
- 移动端点击英雄后会弹出底部详情卡片，避免详情显示在列表最下方看不见
- 英雄 desc 已调整为英雄自身设定/游戏风格描述，不再重复写生活优点和建议，生活相似点、优缺点、建议统一由结果页报告生成
- 响应式适配桌面端与移动端
