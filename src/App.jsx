...
function buildHeroTarget(hero) {
  const target = Object.fromEntries(traits.map((trait) => [trait.key, 0]));
  hero.roles.forEach((role) => {
    const base = roleBase[role] || {};
    traits.forEach(({ key }) => {
      target[key] += base[key] || 0;
    });
  });

  traits.forEach(({ key }) => {
    target[key] = target[key] / hero.roles.length;
  });

  const seed = hashName(hero.name);
  traits.forEach(({ key }, index) => {
    const wobble = (((seed >> (index % 7)) + index * 13) % 5) - 2;
    target[key] = Math.max(0.5, Math.min(10, target[key] + wobble * 0.55));
  });

  // 给部分高辨识英雄一点轻微偏置，让结果更有“本命感”
  const boosts = {
    李白: { creativity: 1.3, adventure: 1.2, independence: 0.8 },
    韩信: { adventure: 1.4, drive: 0.8 },
    诸葛亮: { focus: 1.2, order: 0.8 },
    孙尚香: { drive: 1.1, focus: 0.8 },
    鲁班七号: { focus: 1.2, drive: 0.7 },
    孙悟空: { adventure: 1.1, drive: 1.1 },
    貂蝉: { creativity: 1.1, resilience: 0.9 },
    铠: { resilience: 1.4, independence: 0.8 },
    花木兰: { independence: 1.1, resilience: 0.8 },
    ...
    "元流之子(刺客)": { adventure: 1.4, creativity: 0.8 },
  };

  Object.entries(boosts[hero.name] || {}).forEach(([key, value]) => {
    target[key] = Math.min(10, target[key] + value);
  });
...
function getMatches(scores) {
  ...
  const ranked = heroes
    .map((hero) => {
      ...
      return { ...hero, target, color: getHeroColor(hero), distance };
    })
    .sort((a, b) => a.distance - b.distance);

  const best = ranked[0]?.distance ?? 0;
  const worst = ranked[ranked.length - 1]?.distance ?? 1;
  return ranked.map((hero) => ({
    ...hero,
    match: Math.max(66, Math.min(99, Math.round(99 - ((hero.distance - best) / Math.max(worst - best, 1)) * 31))),
  }));
}
...
function getHeroGameFeature(hero) {
  ...
  return hero.roles.map((role) => featureMap[role]).filter(Boolean).join(" ");
}
      ...
      <main className="page">
        ...
        <section className="hero-pool">
          <div className="section-title">
            ...
            <p>结果匹配会从下面全部英雄中计算，而不是只从少数热门英雄里抽。</p>
          </div>
  ...
  return (
    <div className="result-panel">
      <div className={"result-card " + result.color}>
        ...
        <div className="role-line">{roleText(result.roles)}</div>
